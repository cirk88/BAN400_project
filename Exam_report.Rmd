...
# checking the dictionary we will be using for the sentiment analysis
# first, lets create a frequency table to analyze our data. 



# Then, we decided to choose: hash_sentiment_loughran_mcdonald for our analysis, from the lexicon package
...
positive_words <- df_freq %>%
  filter(df_text_token %in% dictionary.p$x) %>%
  arrange(desc(n)) %>% head(50)

# seeing which negative words are present

negative_words  <- df_freq %>%
  filter(df_text_token %in% dictionary.n$x) %>%
  arrange(desc(n)) %>% head(50)
...
sentiment<-as.vector(unlist(parallel::parLapply(cl, df$texts, sentiment.fun)))
df$sentiment <-sentiment
proc.time()-timesent->ptm
