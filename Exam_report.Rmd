...
# removing stopwords, which are words that are most common in the English language,
# yet carry no meaning. These are found inside the stopwords dictionary in the tm package
         title = title %>% tolower() %>% 
             gsub("[^[:alpha:]]", " ", .) %>% 
             gsub('\\b\\w{1,2}\\s|\\b\\w{21,}\\s','', .) %>% 
             removeWords(., c(stopwords(kind = "en"))))
...
# words carry a different meaning as the paired words may carry a different meaning. 
